...
def fetch_poster(movie_id):
  response = requests.get(
    f"https://api.themoviedb.org/3/movie/{movie_id}?api_key=96aef6c0d373ea579771046d96e72e06"
  )
  data = response.json()
...
def fetch_youtube(movie_id):
  response = requests.get(
    f"http://api.themoviedb.org/3/movie/{movie_id}/videos?api_key=96aef6c0d373ea579771046d96e72e06"
  )
  data = response.json()
...
def fetch_cast(movie_id):
  response = requests.get(
    f"https://api.themoviedb.org/3/movie/{movie_id}?api_key=96aef6c0d373ea579771046d96e72e06&append_to_response=credits"
  )
  data = response.json()
  name = []
  image = []
  for i in data['credits']['cast'][0:5]:
    name.append(i['name'])
    image.append("https://image.tmdb.org/t/p/w600_and_h900_bestv2" +
                 i['profile_path'])
...
def fetch_director(movie_id):
  ...
  for i in data['credits']['crew']:
    if i['job'] == 'Director':
      director_name.append(i['name'])
      director_image.append("https://image.tmdb.org/t/p/w600_and_h900_bestv2" +
                            i['profile_path'])
...
#   string = "This movie recommendation system suggests movies based on the similarity of their content to a movie the user likes. The system analyzes the content of movies (like plot, genre, and actors) to find similar movies. Users input the name of a movie they like, and the system recommends five similar movies based on their content."
#   st.write(string)
    ...
    for i in range(5):
      ...
      with tab1:
        ...
        with col2:
          ...
          with col1:
            st.image(casts[i]['Image'][0], width=70)
            st.write(casts[i]['Name'][0])

          with col2:
            st.image(casts[i]['Image'][1], width=70)
            st.write(casts[i]['Name'][1])

          with col3:
            st.image(casts[i]['Image'][2], width=70)
            st.write(casts[i]['Name'][2])

          with col4:
            st.image(casts[i]['Image'][3], width=70)
            st.write(casts[i]['Name'][3])

          with col5:
            st.image(casts[i]['Image'][4], width=70)
            st.write(casts[i]['Name'][4])

          st.subheader("Director:")
          col1, col2, col3, col4, col5 = st.columns(5)
          with col1:
            st.image(directors[i]['Image'][0], width=70)
            st.write(directors[i]['Name'][0])
